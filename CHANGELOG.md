...
- `zoomImageViewer(uiImage:closeButtonPosition:)` and `zoomImageViewer(uiImage:overlay:)` view modifiers, which replace `ZoomImageView`. The viewer is placed over the view the modifier is attached to.
- `zoomImageViewer(item:image:closeButtonPosition:)` and `zoomImageViewer(item:image:overlay:)` present an optional `Identifiable` and `Equatable` item instead of a `UIImage`, for an overlay that needs more than the image, like a caption. The image is read through a key path.
